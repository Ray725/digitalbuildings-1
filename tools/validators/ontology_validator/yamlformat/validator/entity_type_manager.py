...
class EntityTypeManager(findings_lib.Findings):
  ...
  def _MapFields(self, min_set_size):
    ...
    """
    print('[LOGGING MapFields] :: new run min_set_size', min_set_size)
    current_shard = set()
    type_shards = []
    ...
    for namespace in self._universe.GetNamespaces():
      ...
      print('[LOGGING MapFields] :: namespace valid_types_map', namespace.valid_types_map)
      print('[LOGGING MapFields] :: namespace valid_types_map values', namespace.valid_types_map.values())
      for entity_type in namespace.valid_types_map.values():
        print('[LOGGING MapFields] :: entity_type', entity_type)
        if base_lib.HasAutogeneratedType(
            entity_type.parent_names) or entity_type.is_abstract:
          print('[LOGGING MapFields] :: hit first continue')
          continue
        # TODO address this later for windows
        ...
        # print('[LOGGING MapFields] :: original full_qual_type', '{0}/{1}'.format(ns_name, entity_type.typename))
        ...
        print('[LOGGING MapFields] :: all_qualified_fields', all_qualified_fields)
